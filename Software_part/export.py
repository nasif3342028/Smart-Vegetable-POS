...
try:
    ...
    print("[EXPORT] openpyxl not installed. Excel export disabled. Run: pip install openpyxl")


# Export folder (same directory as the script)
EXPORT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sales_data")
...
class DataExporter:
    """Exports session data to CSV and Excel files."""
    ...
    def export_csv(self, customer_info, cart_items, session_id):
        ...
        Parameters:
            customer_info: dict with 'name' and 'mobile'
            cart_items: list of dicts with 'vegetable', 'weight_kg', 'price_per_kg', 'total'
            session_id: string like 'SES-20260222-091200'
        """
        if not cart_items:
            return
        ...
        try:
            with open(self.csv_path, mode="a", newline="", encoding="utf-8") as f:
                ...
                for i, item in enumerate(cart_items):
                    # Show session total only on the last item row
    ...
    def export_excel(self, customer_info, cart_items, session_id):
        """
        Append session data to the Excel file with professional formatting.
        Creates the file with headers if it does not exist.
        ...
        try:
            ...
            # Add a blank separator row between sessions (if not first data)
            if next_row > 4:
                next_row += 1
            ...
            # Empty cells for remaining columns in total row
    ...
    def export_all(self, customer_info, cart_items, session_id):
        """
        Export session data to both CSV and Excel.
