...
# Database file path (same directory as the script)
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vegetable_pos.db")


class Database:
    """Handles all SQLite operations for the Vegetable POS system."""
    ...
    def save_session(self, customer_info, cart_items):
        """
        Save a complete customer session to the database.

        Parameters:
            customer_info: dict with 'name' and 'mobile'
            cart_items: list of dicts, each with:
                        'vegetable', 'weight_kg', 'price_per_kg', 'total'

        Returns:
            dict with 'customer_id', 'session_id', 'items_saved', 'grand_total'
        """
        if not cart_items:
    ...
    def get_all_sessions(self):
        """
        Retrieve a summary of all sessions.

        Returns:
            list of dicts with session summary info
        """
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_session_details(self, session_id):
        ...
        Returns:
            dict with 'customer' info and 'items' list
        """
        conn = self.get_connection()
        cursor = conn.cursor()
